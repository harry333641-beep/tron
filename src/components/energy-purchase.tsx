...
export function EnergyPurchase({ config }: { config: ServiceConfig }) {
  ...
  return (
    <main id="top" className="min-h-dvh overflow-x-hidden bg-background text-foreground">
      ...
      <section id="steps" className="relative z-10 border-y border-border bg-card/35">
        <div className="mx-auto max-w-5xl px-5 py-10 sm:px-8 sm:py-14">
          <div className="mx-auto max-w-2xl text-center"><SectionEyebrow>BEFORE YOU SEND</SectionEyebrow><h2 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">转 USDT 前的 4 项检查</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">按顺序确认付款地址、到账状态和有效时间，避免能量发放到错误地址。</p></div>
          ...
          <div className="mx-auto mt-5 flex max-w-3xl items-start justify-center gap-3 rounded-xl border border-destructive/30 bg-destructive/10 px-4 py-3 text-center text-sm leading-6 text-destructive"><CircleHelp size={17} className="mt-1 shrink-0" /><p><span className="font-black">重要：</span>如果对方地址的 USDT 余额为 0，请优先选择 131,000 Energy，避免能量不足。</p></div>
        </div>
      </section>
              {checkItems.map(([title, description], index) => <div key={title} className="relative flex gap-4 rounded-xl border border-border bg-card p-4 sm:p-5"><span className="relative z-10 grid size-9 shrink-0 place-items-center rounded-full border border-primary/40 bg-card font-mono text-sm font-bold text-primary shadow-[0_0_0_5px_var(--background)]">{index + 1}</span><div className="min-w-0 text-center"><div className="flex items-center justify-center gap-2"><CheckCircle2 size={16} className="shrink-0 text-accent" /><h3 className="font-black">{title}</h3></div><p className="mt-3 text-sm leading-7 text-muted-foreground">{description}</p></div></div>)}
